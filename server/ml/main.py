        ...
        for img_idx, image_base64 in enumerate(images_base64):
            ...
            for face_idx, face_data in enumerate(faces):
                ...
                try:
                    ...
                    # Check if BEST match is above threshold (ONE FACE = ONE STUDENT ONLY)
                    if best_score >= SIMILARITY_THRESHOLD:
                        ...
                        if usn not in recognized_students:
                            ...
                            existing_confidence = recognized_students[usn]['confidence']
                            existing_priority = emotion_priority.get(existing_emotion, 0)
                            new_priority = emotion_priority.get(emotion, 0)
                            
                            # ALWAYS keep the highest confidence regardless of emotion
                            max_confidence = max(best_score, existing_confidence)
                            ...
                            if new_priority > existing_priority:
                                ...
                                recognized_students[usn]['confidence'] = max_confidence
                                logger.info(f"🔄 Updated {matched_student['name']}: {existing_emotion} → {emotion} (more positive), confidence: {max_confidence:.4f}")
                            elif new_priority == existing_priority and best_score > existing_confidence:
                                recognized_students[usn]['confidence'] = max_confidence
                                logger.info(f"🔄 Updated {matched_student['name']}: confidence {existing_confidence:.4f} → {max_confidence:.4f}")
                            else:
                                # Even if keeping existing emotion, update confidence if higher
                                if max_confidence > existing_confidence:
                                    recognized_students[usn]['confidence'] = max_confidence
                                    logger.info(f"🔄 Updated {matched_student['name']}: confidence {existing_confidence:.4f} → {max_confidence:.4f} (keeping emotion: {existing_emotion})")
                                else:
                                    logger.info(f"ℹ️ Keeping existing data for {matched_student['name']}: {existing_emotion} (priority {existing_priority}) vs {emotion} (priority {new_priority}), confidence: {existing_confidence:.4f}")
            ...
            # Timestamp removed - no longer adding timestamp to images
            logger.info("📷 Processing image without timestamp")
