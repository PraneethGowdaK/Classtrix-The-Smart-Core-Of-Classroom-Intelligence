  ...
  try {
    ...
    if (role === 'teacher') {
      try {
        ...
        if (teacherInfo) {
          // Get all enrollments from teacher_enrollments table with course details (NEW SCHEMA)
      ...
      if (studentInfo) {
        ...
        if (currentEnrollment) {
          additionalInfo = {
            ...
            classId: currentEnrollment.department,
            sectionId: currentEnrollment.section,
      ...
      try {
        ...
        if (teacherInfo) {
          ...
          const enrollments = await getAllRows(
            'SELECT DISTINCT semester, department, section, subject FROM teacher_enrollments WHERE teacher_id = ?',
            [teacherInfo.id]
          );
          
          const semesters = [...new Set(enrollments.map(e => e.semester.toString()))];
          const departments = [...new Set(enrollments.map(e => e.department))];
