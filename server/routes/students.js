...
router.get('/by-class/:classId/:sectionId', authenticateToken, async (req, res) => {
  try {
    ...
    let query = `
      ...
      WHERE se.department = ? AND se.section = ?
    `;
...
router.post('/assign-semester', authenticateToken, requireRole(['admin']), async (req, res) => {
  try {
    const { usn, semester, department, courseIds, enrollmentDate, completionDate } = req.body;

    if (!usn || !semester || !department || !courseIds || courseIds.length === 0) {
      return res.status(400).json({ message: 'Please provide student ID, semester, department, and at least one course' });
    }
    ...
    // Validate enrollment date format (YYYY-MM-DD)
    if (!/^\d{4}-\d{2}-\d{2}$/.test(enrollmentDate)) {
      return res.status(400).json({ message: 'Enrollment date must be in YYYY-MM-DD format' });
    }

    // Validate completion date format (YYYY-MM-DD)
    ...
    // Check 2: Validate all course IDs exist and match department/semester
    const courseIdArray = courseIds.map(id => parseInt(id));
    const placeholders = courseIdArray.map(() => '?').join(',');
    
    const validCourses = await getAllRows(
      `SELECT id, course_code, department, semester, section FROM courses 
       WHERE id IN (${placeholders}) AND department = ? AND semester = ?`,
      [...courseIdArray, department, parseInt(semester)]
    );

    if (validCourses.length !== courseIdArray.length) {
      return res.status(400).json({ 
        message: 'Some selected courses do not match the specified department or semester' 
      });
    }

    // Check 3: If year-month already exists for this student
    const enrollmentYearMonth = enrollmentDate.substring(0, 7); // Gets YYYY-MM
    const existingEnrollment = await getRow(
      'SELECT id, enrollment_date FROM student_enrollments WHERE student_id = ? AND enrollment_date LIKE ?',
    ...
    // Check 4: If this specific semester already exists for this student
    ...
    // Check 5: Check for duplicate course enrollments for this student
    const existingCourseEnrollments = await getAllRows(
      `SELECT c.course_code FROM student_enrollments se 
       JOIN courses c ON se.course_id = c.id 
       WHERE se.student_id = ? AND se.course_id IN (${placeholders})`,
      [student.id, ...courseIdArray]
    );

    if (existingCourseEnrollments.length > 0) {
      const duplicateCourses = existingCourseEnrollments.map(c => c.course_code).join(', ');
      return res.status(409).json({ 
        message: `Student is already enrolled in some courses: ${duplicateCourses}`
      });
    }
    ...
    for (const courseId of courseIdArray) {
      const result = await runQuery(
        'INSERT INTO student_enrollments (student_id, course_id, enrollment_date, completion_date, created_at) VALUES (?, ?, ?, ?, ?)',
        [student.id, courseId, enrollmentDate, completionDate, localTimestamp]
      );
...
// GET /api/students/enrolled - Get students enrolled in a specific course
// IMPORTANT: This route must be BEFORE /:usn to avoid route conflict
router.get('/enrolled', authenticateToken, async (req, res) => {
  try {
    const { courseId } = req.query;

    console.log('📚 Get enrolled students request for courseId:', courseId);
    ...
    // Get all students enrolled in this course (currently active enrollments)
...
router.get('/:usn/enrollments', authenticateToken, requireRole(['teacher', 'admin']), async (req, res) => {
  try {
    ...
    const enrollments = await getAllRows(
      `SELECT se.id, c.semester, c.department, c.section, c.course_code as subject_code, c.course_name as subject_name, 
              se.enrollment_date, se.completion_date, se.course_id
       FROM student_enrollments se
       JOIN courses c ON se.course_id = c.id
       WHERE se.student_id = ?
       ORDER BY se.enrollment_date DESC, c.semester ASC`,
      [student.id]
    );
