...
export default function ManageTimetable() {
  ...
  // Dynamic data state for Create Timetable tab
  const [departments, setDepartments] = useState<string[]>([]);
  const [allSemesters, setAllSemesters] = useState<string[]>([]);
  const [templateSemesters, setTemplateSemesters] = useState<string[]>([]);
  const [templateSections, setTemplateSections] = useState<string[]>([]);
  const [templateCourses, setTemplateCourses] = useState<any[]>([]);

  // Dynamic data state for View Timetable filters
  const [filterSections, setFilterSections] = useState<string[]>([]);

  // Dynamic data state for Update Timetable tab
  const [updateSemesters, setUpdateSemesters] = useState<string[]>([]);
  const [updateDepartments, setUpdateDepartments] = useState<string[]>([]);
  const [updateSections, setUpdateSections] = useState<string[]>([]);
  const [updateCourses, setUpdateCourses] = useState<any[]>([]);

  // Fetch all semesters and departments on mount
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        // Fetch all unique semesters
        const semResponse = await fetch(`${API_BASE_URL}/courses/semesters?department=CS`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (semResponse.ok) {
          const semData = await semResponse.json();
          const allSems = semData.semesters || [];
          setAllSemesters(allSems);
          setTemplateSemesters(allSems);
        }

        // Fetch departments
        const deptResponse = await fetch(`${API_BASE_URL}/courses/departments`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (deptResponse.ok) {
  ...
  // Fetch sections when semester and department are selected
  useEffect(() => {
    const fetchSections = async () => {
      if (!templateDepartment || !templateSemester) {
        setTemplateSections([]);
        return;
      }
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/courses/sections?department=${templateDepartment}&semester=${templateSemester}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
    ...
    fetchSections();
  }, [templateDepartment, templateSemester]);

  // Fetch courses when template filters change
  useEffect(() => {
    const fetchCourses = async () => {
      if (!templateDepartment || !templateSemester || !templateSection) {
        setTemplateCourses([]);
        return;
      }
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/courses/filtered?department=${templateDepartment}&semester=${templateSemester}&section=${templateSection}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });

        if (response.ok) {
          const data = await response.json();
          setTemplateCourses(data.courses || []);
        }
      } catch (error) {
    ...
    fetchCourses();
  }, [templateDepartment, templateSemester, templateSection]);

  // Fetch departments for View Timetable when semester changes
  useEffect(() => {
    const fetchFilterDepartments = async () => {
      if (!timetableFilter.semester) {
        return;
      }
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/courses/departments?semester=${timetableFilter.semester}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });

        if (response.ok) {
          const data = await response.json();
          // departments state is already set globally, no need to update
        }
      } catch (error) {
        console.error('Failed to fetch filter departments:', error);
      }
    };
  ...
  // Fetch sections for View Timetable when semester and department change
  useEffect(() => {
    const fetchFilterSections = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/courses/sections?department=${timetableFilter.department}&semester=${timetableFilter.semester}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        console.error('Failed to fetch filter sections:', error);
      }
    };
  ...
  // Reset timetable display when component mounts
  useEffect(() => {
  ...
  useEffect(() => {
    if (activeTab === "view") {
      // Reset View Timetable tab
      setTimetableFilter({ semester: "", department: "", section: "" });
      setTimetableLoaded(false);
      setTimetableList([]);
      setFilterSections([]);
    } else if (activeTab === "create") {
      // Reset Create Timetable tab
      setTemplateSemester("");
      setTemplateDepartment("");
      setTemplateSection("");
      setTemplateDay("");
      setTemplateDuration("90");
      setQuickEntries([{ courseId: "", dayOfWeek: "", startTime: "", endTime: "" }]);
      setTemplateSections([]);
      setTemplateCourses([]);
      setCSVFile(null);
      ...
      setUpdateSource('default');
      setUpdateDayOfWeek("");
      setUpdateLoading(false);
      setUpdateLoaded(false);
      setUpdateSections([]);
      setUpdateCourses([]);
      setUpdatePeriodDuration("60");
    }
  }, [activeTab]);
  ...
  const addQuickEntryRow = () => {
    ...
    setQuickEntries([...quickEntries, { courseId: "", dayOfWeek: templateDay, startTime: newStartTime, endTime: newEndTime }]);
  };
  ...
  const handleQuickEntrySubmit = async () => {
    ...
    const validEntries = quickEntries.filter(entry => 
      entry.courseId && entry.startTime && entry.endTime
    );
    ...
    for (const entry of validEntries) {
      try {
        ...
        const fullEntry = {
          courseId: parseInt(entry.courseId),
    ...
    setQuickEntries([{ courseId: "", dayOfWeek: templateDay, startTime: "", endTime: "" }]);
    setSubmitting(false);
      ...
      try {
        ...
        const entries = lines.slice(1).map(line => {
          ...
          return { 
            ...
            department: parts[1], 
            section: parts[2], 
            courseCode: parts[3], // Changed from subject to courseCode
            dayOfWeek: parts[4], 
            startTime: parts[5], 
            endTime: parts[6],
        ...
        for (const entry of entries) {
          if (!entry.semester || !entry.department || !entry.section || !entry.courseCode || !entry.dayOfWeek || !entry.startTime || !entry.endTime) {
            errorCount++;
          ...
          try {
            ...
            // Look up courseId from course_code, semester, department, section
            const courseResponse = await fetch(
              `${API_BASE_URL}/courses/filtered?semester=${entry.semester}&department=${entry.department}&section=${entry.section}`,
              { headers: { 'Authorization': `Bearer ${token}` } }
            );

            if (!courseResponse.ok) {
              errorCount++;
              errors.push(`Failed to fetch courses for ${entry.courseCode}`);
              continue;
            }

            const courseData = await courseResponse.json();
            const course = courseData.courses?.find((c: any) => c.course_code === entry.courseCode);

            if (!course) {
              errorCount++;
              errors.push(`Course not found: ${entry.courseCode} for Sem ${entry.semester}, ${entry.department}, Section ${entry.section}`);
              continue;
            }

            console.log('Sending entry with courseId:', course.id);
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <CardContent className="pt-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <Select value={timetableFilter.semester} onValueChange={(value) => setTimetableFilter(prev => ({ ...prev, semester: value, department: "", section: "" }))}>
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select value={timetableFilter.department} onValueChange={(value) => setTimetableFilter(prev => ({ ...prev, department: value, section: "" }))}>
                    <SelectTrigger>
            ...
            <div className="flex gap-3">
              ...
              <div className="space-y-1">
                ...
                <ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1 list-disc list-inside">
                  <li>Avoid time slot overlaps for the same day, semester, department, and section</li>
                  <li>Do not create duplicate timetable entries for the same day and time</li>
                  <li>Verify all entries before submission to prevent scheduling conflicts</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4 bg-muted rounded-lg">
                <div>
                  <Label>Semester <span className="text-red-500">*</span></Label>
                  <Select value={templateSemester} onValueChange={(value) => { setTemplateSemester(value); setTemplateSection(""); }}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select semester" />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  <Label>Department <span className="text-red-500">*</span></Label>
                  <Select value={templateDepartment} onValueChange={(value) => { setTemplateDepartment(value); setTemplateSection(""); }}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select department" />
                    </SelectTrigger>
                    <SelectContent>
                        ...
                        {quickEntries.map((entry, index) => (
                          <tr key={index} className="border-b hover:bg-muted/50">
                            <td className="border p-2">
                              <Select value={entry.courseId} onValueChange={(value) => handleQuickEntryChange(index, 'courseId', value)}>
                                <SelectTrigger className="h-8">
                                  <SelectValue placeholder="Select course" />
                                </SelectTrigger>
                                <SelectContent>
                                  {templateCourses.map(course => (
                                    <SelectItem key={course.id} value={course.id.toString()}>{course.course_code} - {course.course_name}</SelectItem>
                                  ))}
                                </SelectContent>
                              </Select>
                            </td>
              ...
              <div className="space-y-4">
                <div className="bg-muted p-3 rounded-lg space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground font-mono bg-white dark:bg-gray-800 p-2 rounded border">semester,department,section,courseCode,dayOfWeek,startTime,endTime</p>
                  </div>
                  ...
                  <div className="border-t pt-2">
                    ...
                    <ul className="text-sm text-muted-foreground space-y-0.5 ml-4">
                      ...
                      <li>• Course Code: EXACT code from system (e.g., CS101, IS201)</li>
                  ...
                  <div className="border-t pt-2">
                    ...
                    <div className="text-sm text-muted-foreground font-mono bg-white dark:bg-gray-800 p-2 rounded border space-y-0.5">
                      <div>semester,department,section,courseCode,dayOfWeek,startTime,endTime</div>
                      <div>1,CS,A,CS101,Monday,09:00,09:55</div>
                    </div>
                  </div>
                  ...
                  <div className="p-2 bg-blue-50 dark:bg-blue-950/20 rounded border border-blue-300 dark:border-blue-900">
                    <p className="text-sm text-blue-900 dark:text-blue-100 flex items-start gap-1">
                      ...
                      <span><strong>Note:</strong> First row = header. Course codes must match system exactly. The course must exist in the database for the specified semester, department, and section. Save as CSV (Comma delimited).</span>
                    </p>
                  </div>
                </div>
                <div>
        ...
        <TabsContent value="update" className="mt-4 space-y-4">
          <UpdateTimetableContent 
            ...
            setUpdateSemesters={setUpdateSemesters}
...
function UpdateTimetableContent({ 
  ...
  updateSemesters, setUpdateSemesters,
  updateDepartments, setUpdateDepartments,
  updateSections, setUpdateSections,
  updateCourses, setUpdateCourses,
  allSemesters,
  departments,
  toast
}: any) {
  ...
  // Fetch departments when semester changes
  useEffect(() => {
    if (!updateSemester) {
      setUpdateDepartments([]);
      return;
    }
    // Use the global departments list
    setUpdateDepartments(departments);
  }, [updateSemester, departments]);

  // Fetch sections when semester and department change
  useEffect(() => {
    const fetchUpdateSections = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/courses/sections?department=${updateDepartment}&semester=${updateSemester}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        console.error('Failed to fetch update sections:', error);
      }
    };
  ...
  useEffect(() => {
    const fetchUpdateCourses = async () => {
      if (!updateSemester || !updateDepartment || !updateSection) {
        setUpdateCourses([]);
        return;
      }
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/courses/filtered?department=${updateDepartment}&semester=${updateSemester}&section=${updateSection}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });

        if (response.ok) {
          const data = await response.json();
          setUpdateCourses(data.courses || []);
        }
      } catch (error) {
    ...
    fetchUpdateCourses();
  }, [updateSemester, updateDepartment, updateSection]);
  ...
  const loadSchedule = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Transform the timetable data to match our frontend structure
        const transformedTimetable = (data.timetable || []).map((entry: any) => ({
          start_time: entry.start_time,
          end_time: entry.end_time,
          courseId: entry.course_id ? entry.course_id.toString() : (entry.courseId ? entry.courseId.toString() : "")
        }));
        
        setUpdateTimetable(transformedTimetable);
        ...
        toast({
          ...
          description: `Showing ${data.source} timetable for ${data.day_of_week}. ${transformedTimetable.length} periods found.`
        });
      } else {
        toast({
  ...
  const saveSchedule = async () => {
    // Validation
    const emptyCourses = updateTimetable.filter(entry => !entry.courseId || entry.courseId === '');
    if (emptyCourses.length > 0) {
      toast({
        title: "Validation Error",
        description: "All periods must have a course selected",
        variant: "destructive"
      });
      return;
    }
        ...
        <CardContent className="space-y-4">
          ...
          <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            <div className="flex gap-3">
              ...
              <div className="space-y-1">
                <p className="text-sm font-semibold text-blue-900 dark:text-blue-100">Important Notes</p>
                <ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1 list-disc list-inside">
                  <li>Each date can only be updated once for a specific semester, department, and section</li>
                  <li>Ensure no scheduling conflicts across the entire academic timetable</li>
                  <li>Changes will overwrite existing timetable entries for the selected date</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={updateSemester} onValueChange={(value) => { setUpdateSemester(value); setUpdateDepartment(""); setUpdateSection(""); }} disabled={updateLoaded}>
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select value={updateDepartment} onValueChange={(value) => { setUpdateDepartment(value); setUpdateSection(""); }} disabled={updateLoaded}>
                <SelectTrigger>
                  <SelectValue placeholder="Select" />
                </SelectTrigger>
                <SelectContent>
                ...
                <tbody className="divide-y">
                  {updateTimetable.length === 0 ? (
                    ...
                    updateTimetable.map((entry: any, index: number) => {
                      const selectedCourse = updateCourses.find(c => c.id.toString() === entry.courseId);
                      return (
                          ...
                          <td className="px-4 py-3">
                            {updateIsLocked ? (
                              <span className="text-sm">
                                {selectedCourse ? `${selectedCourse.course_code} - ${selectedCourse.course_name}` : entry.courseId}
                              </span>
                            ) : (
                              <Select
                                value={entry.courseId}
                                onValueChange={(value) => updatePeriod(index, 'courseId', value)}
                              >
