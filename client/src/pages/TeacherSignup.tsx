...
import { SEMESTERS, DEPARTMENTS, SECTIONS, API_BASE_URL, getSubjectsByDepartments } from "@/lib/constants";
...
export default function TeacherSignup() {
  ...
  const filteredSubjects = useMemo(() => {
    ...
    formData.semesters.forEach(semester => {
      const subjectsByDept = getSubjectsByDepartments(semester, formData.departments);
      result[semester] = subjectsByDept;
    });
    
    return result;
  }, [formData.semesters, formData.departments]);
  
  // Create subject groups for display
  const subjectGroups = useMemo(() => {
    const groups: Record<string, string[]> = {};
    
    Object.entries(filteredSubjects).forEach(([semester, deptSubjects]) => {
      Object.entries(deptSubjects).forEach(([dept, subjects]) => {
        const key = `Semester ${semester}`;
        if (!groups[key]) {
          groups[key] = [];
        }
        groups[key].push(...subjects);
      });
    });
